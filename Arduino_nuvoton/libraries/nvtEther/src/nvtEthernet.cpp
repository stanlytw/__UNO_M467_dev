...
int EthernetClass::begin(unsigned long timeout, unsigned long responseTimeout)
{
    ...
    _dhcp = &s_dhcp;
    nvt_eth_init(NULL, NULL, NULL, NULL);
...
{
    nvt_eth_init(NULL, local_ip.raw_address(), gateway.raw_address(), subnet.raw_address());
    /* If there is a local DHCP informs it of our manual IP configuration to
    prevent IP conflict */
    nvt_DHCP_manual_config();
...
int EthernetClass::begin(uint8_t *mac_address, unsigned long timeout, unsigned long responseTimeout)
{
    ...
    nvt_eth_init(mac_address, NULL, NULL, NULL);
...
{
    nvt_eth_init(mac_address, local_ip.raw_address(), gateway.raw_address(), subnet.raw_address());
    /* If there is a local DHCP informs it of our manual IP configuration to
    prevent IP conflict */
    nvt_DHCP_manual_config();
    _dnsServerAddress = dns_server;
...
{
    return (!nvt_eth_is_init()) ? Unknown : (nvt_eth_link_up() ? LinkON : LinkOFF);
}
...
{
    nvt_eth_set_macaddr(mac_address);
}

void EthernetClass::MACAddress(uint8_t *mac_address)
{
    nvt_eth_get_macaddr(mac_address);
}

IPAddress EthernetClass::localIP()
{
    return IPAddress(nvt_eth_get_ipaddr());
}

IPAddress EthernetClass::subnetMask()
{
    return IPAddress(nvt_eth_get_netmaskaddr());
}

IPAddress EthernetClass::gatewayIP()
{
    return IPAddress(nvt_eth_get_gwaddr());
}
