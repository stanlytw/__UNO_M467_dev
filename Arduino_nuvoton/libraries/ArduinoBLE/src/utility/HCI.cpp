...
void HCIClass::poll(unsigned long timeout)
{
  ...
  if (HCITransport.available()) {
    uint16_t u16RcvLen = HCINuvotonTransport.getRcvLen();
    HCINuvotonTransport.readRawData2ParserBuf(_recvBuffer, sizeof(_recvBuffer));
    HCINuvotonTransport.cb_clrDataReady();

    if (_recvBuffer[0] == HCI_ACLDATA_PKT) {
      handleAclDataPkt(u16RcvLen, &_recvBuffer[1]);
    } else if (_recvBuffer[0] == HCI_EVENT_PKT) {
      handleEventPkt(u16RcvLen, &_recvBuffer[1]);
    } else {
      if (_debug) {
        dumpPkt("debug pkt: ", sizeof(_recvBuffer), _recvBuffer);
      }
    }
  }
#else
  #error not support ArduinoBLE
#endif
