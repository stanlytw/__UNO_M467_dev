...
static struct nvt_eth_config gconfig;
...
{
    static uint32_t gEhtLinkTickStart = 0;
#define TIME_CHECK_ETH_LINK_STATE 500

    /* Check ethernet link status */
    if ((sys_now() - gEhtLinkTickStart) >= TIME_CHECK_ETH_LINK_STATE)
    {
        //ethernetif_set_link(&gnetif);
        gEhtLinkTickStart = sys_now();
    }
...
#if LWIP_DHCP
    nvt_DHCP_Periodic_Handle(&gnetif);
#endif /* LWIP_DHCP */
}
/**
...
int8_t nvt_dns_gethostbyname(const char *hostname, uint32_t *ipaddr)
{
    ...
    *ipaddr = 0;
    ...
    case ERR_INPROGRESS:
        ...
        {
            nvt_eth_scheduler();
            if ((sys_now() - tickstart) >= TIMEOUT_DNS_REQUEST)
            {
                ret = -1;
                break;
            }
...
uint16_t nvt_get_data(struct pbuf_data *data, uint8_t *buffer, size_t size)
{
    ...
    while ((nb < size) && (data->p != NULL) && (data->available > 0))
    {
        ...
        if (nb < size)
        {
            ...
            ptr = nvt_free_data(ptr);
        }
    }

    if (data->available == 0)
    {
        data->p = nvt_free_data(data->p);
    }
