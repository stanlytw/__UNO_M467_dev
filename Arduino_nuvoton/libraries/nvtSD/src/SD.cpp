...
{
    nvtDbg_printf("SD class Error_handler!!\n");
}
/**
...
{
  nvtDbg_printf("openRoot, root path:%s\n", _fatFs.getRoot());
...
void File::close()
{
  if (_name) {
    ...
    if (_fil) {
      ...
      {
         nvtDbg_printf("_fil->fs==0!!\n");
      }
      free(_fil);
      _fil = NULL;
      nvtDbg_printf("File close and free!!\n");
    }
