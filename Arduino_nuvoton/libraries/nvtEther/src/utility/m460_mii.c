...
int32_t mii_ethtool_gset(synopGMACdevice *gmacdev, uint8_t reset)
{
    ...
    if (reset)
    {
        ...
        if (ret < 0)
        {
            //nvtEthernet_printf("mii:: Reset PHY, FAIL.\n");
            return -ESYNOPGMACPHYERR;
        }

        //nvtEthernet_printf("mii:: Reset PHY, PASS.\n");
    }
    ...
    if (ret < 0)
    {
        nvtEthernet_printf("mii:: Read MII_BMSR status, FAIL.\n");
        return -ESYNOPGMACPHYERR;
    }
    ...
    if (val & LPA_100FULL)
    {
        nvtEthernet_printf("mii:: 100M FULLDUPLEX\n");
        gmacdev->DuplexMode = FULLDUPLEX;
        gmacdev->Speed      = SPEED100;
    }
    else if (val & LPA_100HALF)
    {
        nvtEthernet_printf("mii:: 100M HALFDUPLEX\n");
        gmacdev->DuplexMode = HALFDUPLEX;
        gmacdev->Speed      = SPEED100;
    }
    else if (val & LPA_10FULL)
    {
        nvtEthernet_printf("mii:: 10M FULLDUPLEX\n");
        gmacdev->DuplexMode = FULLDUPLEX;
        gmacdev->Speed      = SPEED10;
    }
    else if (val & LPA_10HALF)
    {
        nvtEthernet_printf("mii:: 10M HALFDUPLEX\n");
        gmacdev->DuplexMode = HALFDUPLEX;
        gmacdev->Speed      = SPEED10;
    }
    else
    {
        nvtEthernet_printf("mii:: 100M FULLDUPLEX (Default: LPA 0x%x)\n", val);
        gmacdev->DuplexMode = FULLDUPLEX;
        gmacdev->Speed      = SPEED100;
    }
...
void mii_link_monitor(synopGMACdevice *gmacdev)
{
    if (mii_link_ok(gmacdev) != LINKUP)
    {
        ...
        {
            nvtEthernet_printf("mii:: Link down\n");
        }
        gmacdev->DuplexMode = 0;
        gmacdev->Speed      = 0;
        gmacdev->LinkState  = 0;
    ...
    else
    {
        if (!gmacdev->LinkState)
        {
            ...
            nvtEthernet_printf("mii:: Link up\n");
        }
    }
}
