...
/** @addtogroup NUC131_CAN_Driver CAN Driver
  @{
*/

/** @addtogroup NUC131_CAN_EXPORTED_FUNCTIONS CAN Exported Functions
  @{
*/
...
/**
  * @brief The function is used to configure a receive message object with ID filter and Mask.
  *
  * @param[in] tCAN The pointer to CAN module base address.
  * @param[in] u32MsgNum Specifies the Message object number from 0 to 31.
  * @param[in] u32IDType Specifies the identifier type of the frames that will be transmitted. Valid values are:
  ...
  *                      - CAN_EXT_ID: The 29-bit identifier.
  * @param[in] u32ID Specifies the identifier used for acceptance filtering.
  * @param[in] u32ID_Mask Specifies the identifier's mask used for acceptance filtering.
  *											 - CAN_ID_MASK_ALLPASS: All pass 
  * @retval FALSE No useful interface.
  * @retval TRUE Configure a receive message object success.
  *
  * @details If the RxIE bit (CAN_IFn_MCON[10]) is set, the IntPnd bit (CAN_IFn_MCON[13])
...
int32_t CAN_SetRxMsgAndMsk(CAN_T *tCAN, uint32_t u32MsgNum , uint32_t u32IDType, uint32_t u32ID, uint32_t u32ID_Mask)
{
    uint32_t u32TimeOutCount = 0;

    while(CAN_SetRxMsgObjAndMsk(tCAN, u32MsgNum, u32IDType, u32ID, u32ID_Mask,TRUE) == FALSE)
    {
        u32TimeOutCount++;
...
/*@}*/ /* end of group NUC131_CAN_EXPORTED_FUNCTIONS */

/*@}*/ /* end of group NUC131_CAN_Driver */
